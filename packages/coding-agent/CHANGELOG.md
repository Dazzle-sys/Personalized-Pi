...
- Removed the fullscreen right-click paste handler. In fullscreen mode the terminal forwards mouse events to the TUI (SGR mouse reporting is on), so right-click no longer pastes; use keyboard paste (Ctrl+V / Shift+Insert). Right-clicking an active text selection copies it to the clipboard.
...
- Added usage notices for compaction and branch summaries when cache miss notices are enabled.
