...
import { setCapabilityOverrides, t } from "@earendil-works/pi-tui";
...
import { applyLocaleSetting, setLocale } from "./i18n/index.ts";
...
function reportDiagnostics(diagnostics: readonly AgentSessionRuntimeDiagnostic[]): void {
	for (const diagnostic of diagnostics) {
		...
		const prefix =
			diagnostic.type === "error" ? `${t("Error")}: ` : diagnostic.type === "warning" ? `${t("Warning")}: ` : "";
...
async function runAuthCommand(args: string[]): Promise<boolean> {
	...
	if (parsed.unknownFlags.size > 0) {
		...
		const commandName = getAuthCommandName(command.kind);
		const usage = getAuthCommandUsage(command.kind);
		console.error(chalk.red(t('Unknown option --{option} for "{command}".', { option, command: commandName })));
		console.error(chalk.dim(t('Use "{app} --help" or "{usage}".', { app: APP_NAME, usage })));
		process.exitCode = 1;
		return true;
	}
	try {
...
export async function main(args: string[], options?: MainOptions) {
	resetTimings();
	// Resolve the UI locale from the environment before any user-visible output.
	applyLocaleSetting();
	...
	// The explicit language setting (if any) overrides the environment locale.
	applyLocaleSetting(bootstrapSettingsManager.getGlobalSettings().language);
	...
	// print/JSON 模式的 stdout/stderr 供脚本消费，系统消息不随 locale 变化；助手正文语言由模型决定。
	...
	if (hasRuntimeErrors) {
		// Detect extension-load failures in the active locale: the diagnostic message is
		// translated, so match on the localized prefix before the "{path}" placeholder.
		const extensionLoadFailureMarker = t('Failed to load extension "{path}": {error}').split("{path}")[0];
		if (runtime.diagnostics.some((diagnostic) => diagnostic.message.includes(extensionLoadFailureMarker))) {
