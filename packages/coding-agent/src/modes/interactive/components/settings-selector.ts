...
// Labels are translated at call time (locale may change after module load)
function defaultProjectTrustLabels(): Record<DefaultProjectTrust, string> {
	return {
		ask: t("Ask"),
		always: t("Always trust"),
		never: t("Never trust"),
	};
}
...
export class SettingsSelectorComponent extends Container {
	...
	constructor(config: SettingsConfig, callbacks: SettingsCallbacks) {
		...
		const trustLabels = defaultProjectTrustLabels();
		const trustByLabel = new Map(
			Object.entries(trustLabels).map(([value, label]) => [label, value as DefaultProjectTrust]),
		);
		...
		const items: SettingItem[] = [
			...
			},
			{
				...
				currentValue: trustLabels[config.defaultProjectTrust],
				values: Object.values(trustLabels),
			},
			{
			...
			},
			{
				...
				description: t("Override the default thinking level for specific models. {keys} cycles in-session.", {
					keys: cycleThinkingKey,
				}),
			...
			},
			{
				...
				label: t("Fullscreen copy on select"),
				description: t(
			...
			},
			{
				id: "language",
				label: t("Language"),
				description: t("Interface language; Auto follows the system locale (PI_LOCALE/LANG)"),
				currentValue: config.language,
				values: ["auto", "en", "zh-CN"],
			},
			{
				id: "theme",
				label: t("Theme"),
			...
			(id, newValue) => {
				switch (id) {
					...
					case "default-project-trust": {
						const defaultProjectTrust = trustByLabel.get(newValue);
